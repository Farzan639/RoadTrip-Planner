...
const EditTripPage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.title.trim() || !formData.description.trim() || !formData.startDestination.trim() || !formData.finalDestination.trim()) {
        ...
        const route = [{ locationName: formData.startDestination.trim(), description: "Starting Point" }, { locationName: formData.finalDestination.trim(), description: "Final Destination" }];
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-900">
                <div className="text-center">
                    ...
                    <p className="text-slate-400 text-lg font-medium">Loading Trip Editor...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-slate-300 py-12">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-8">
                    <button onClick={() => navigate(-1)} className="mb-4 flex items-center text-slate-300 hover:text-cyan-400 transition-colors duration-200 font-semibold">
                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                        Back
                    </button>
                    <h1 className="text-4xl font-bold text-white mb-4">
                        Edit Your <span className="text-cyan-400">Road Trip</span>
                    </h1>
                    <p className="text-lg text-slate-400">Update the details of your adventure.</p>
                </div>
                ...
                <div className="bg-slate-800 rounded-2xl shadow-2xl p-8 border border-slate-700">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <input id="title" type="text" name="title" value={formData.title} onChange={handleChange} required className="input-field" />
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="startDestination" className="block text-sm font-semibold text-slate-300 mb-2">Starting Point *</label>
                                <input id="startDestination" type="text" name="startDestination" value={formData.startDestination} onChange={handleChange} required className="input-field" />
                            </div>
                            <div>
                                <label htmlFor="finalDestination" className="block text-sm font-semibold text-slate-300 mb-2">Final Destination *</label>
                                <input id="finalDestination" type="text" name="finalDestination" value={formData.finalDestination} onChange={handleChange} required className="input-field" />
                            </div>
                        </div>

                        <div>
                            <label htmlFor="description" className="block text-sm font-semibold text-slate-300 mb-2">Trip Description *</label>
                            <textarea id="description" name="description" rows="4" value={formData.description} onChange={handleChange} required className="input-field resize-none" />
                        </div>

                        <div>
                            <label htmlFor="images" className="block text-sm font-semibold text-slate-300 mb-2">Upload New Images (Optional)</label>
                            <div className="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center hover:border-cyan-500 transition-colors duration-200">
                                ...
                                <label htmlFor="images" className="cursor-pointer">
                                    <div className="space-y-2">
                                        ...
                                        <div>
                                            <p className="text-slate-400">Click to replace images</p>
                                            <p className="text-xs text-slate-500">This will overwrite existing images</p>
                                        </div>
                                    </div>
                                </label>
                            </div>
                            {images && images.length > 0 && <div className="mt-2 text-sm text-emerald-400">{images.length} new image{images.length > 1 ? 's' : ''} selected</div>}
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="duration" className="block text-sm font-semibold text-slate-300 mb-2">Estimated Duration</label>
                                <input id="duration" type="text" name="duration" value={formData.duration} onChange={handleChange} className="input-field" />
                            </div>
                            <div>
                                <label htmlFor="difficulty" className="block text-sm font-semibold text-slate-300 mb-2">Difficulty</label>
                                <select id="difficulty" name="difficulty" value={formData.difficulty} onChange={handleChange} className="input-field">
                                    <option>Easy</option>
                        ...
                        <div>
                            ...
                            <input id="tags" type="text" name="tags" value={formData.tags} onChange={handleChange} className="input-field" />
                            <p className="text-xs text-slate-500 mt-1">Separate tags with commas.</p>
                        </div>
                        
                        <div className="pt-6">
                            <button type="submit" disabled={saving} className={`w-full py-4 px-6 rounded-lg font-semibold text-lg transition-all duration-300 ${saving ? 'bg-slate-600 cursor-not-allowed' : 'bg-cyan-500 text-slate-900 hover:bg-cyan-400 transform hover:scale-105'}`}>
                                {saving ? (
